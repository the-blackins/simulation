...
class States:
    ...
    def create_memory_state(self, mem_dict):
    ...
    def state_wrapper(self, mem_list):
        from app import db
        try:
            with db.session.no_autoflush:
                # get the total number of simulations in the simulation  database
                for mem_dict in mem_list:
                    mermory_state= self.create_memory_state(mem_dict)
                    sim_states = States(self, mermory_state)
                return sim_states
        except Exception as e:
